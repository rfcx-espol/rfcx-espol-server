...
var dataL=[];
var ind=0;
var deviceName=$("#nameDevice").text();
var deviceId = parseInt($("#deviceId").text());
...
function getData(){
    $.get('api/Device/'+parseInt(deviceId)+'/Sensor/',getSensors);
}
...
function getSensors(data){
    ...
    for( sensor of sensors){
        ...
        if(type.includes("Hum") && location.includes("Amb")) {
            var idMin = "minMonHum";
            var idMax = "maxMonHum";
            var idAvg = "avgMonHum";
        ...
        }else if(type.includes("Temp") && location.includes( "Dev")){
            var idMin = "minMonDis";
        ...
        }else if(type.includes("Temp") && location.includes("Amb")){
            var idMin = "minMonAmb";
            var idMax = "maxMonAmb";
            var idAvg = "avgMonAmb";
...
function displayMonitor() {
    for (sensors of sensorsList){
        ...
        $.getJSON('api/Device/'+deviceId+'/Sensor/'+idSensor+query, addData);
    }
...
function addData(data) {
    ...
    if(typeS.includes("Temp") && locationS.includes("Dev")){
        var colorP = "#424084";
        var idMin = "minMonDis";
        var idMax = "maxMonDisp";
        var idAvg = "avgMonDisp";
        var divIdChart = "chartMonitorDisp";
    }else if(typeS.includes("Temp") && locationS.includes("Amb")){
        var colorP = "orange";
        var idMin = "minMonAmb";
        var idMax = "maxMonAmb";
        var idAvg = "avgMonAmb";
        var divIdChart = "chartMonitorAmb";
    }else if(typeS.includes("Hum") && locationS.includes("Amb")){
...
function individualChart(nameChart){
    if(nameChart.includes("Temp") && nameChart.includes("Amb")){
        var minVal = "minValueAmb";
        var maxVal = "maxValueAmb";
        var avgVal = "avgValueAmb";
        var idDiv = "temp_amb";
    }else if(nameChart.includes("Temp") && nameChart.includes("Dev")){
        var minVal = "minValueTemp";
        var maxVal = "maxValueTemp";
        var avgVal = "avgValueTemp";  
        var idDiv = "temp_disp";  
    }else if(nameChart.includes("Hum") && nameChart.includes("Amb")){
    ...
    '<h4 class="chart-title"> '+deviceName+"/ "+nameChart+' </h4>'+
...
function startDisplayEachChart(id) {
    ...
    var query = '/DataTimestamp?startTimestamp='+lastTimestamp+'&endTimestamp='+current;
    $.getJSON('api/Device/'+deviceId+'/Sensor/'+id+query, addDataEachChart);
}
