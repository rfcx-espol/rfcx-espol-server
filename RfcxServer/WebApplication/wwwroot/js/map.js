		...
		$.get('api/Station/'+parseInt(stationId)+'/Sensor/', function(data){
			...
			for(sensor of dataDic){
				...
				if(typeSensor=="Humedad"){
					...
					stations[idStation-1]["content"] = contentS + '<p style="font-size: 14px; float: left;"><i class="fa fa-thermometer" style="font-size:20px; color: #ff7800;"></i> Amb.: </p><p class="valueTempAmb" id="tempAmbId" style="font-size: 14px; line-height: 2;"> 35 °C</p>';
				...
				stations[idStation-1]["content"] = stations[idStation-1]["content"] +'</div>'+'</div>';
